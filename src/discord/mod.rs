...
impl CommandService {
    ...
    pub async fn play(&self, guild_id: u64, user_id: u64, query: &str) -> Result<String, String> {
        ...
        let mut s = self.sessions.get_or_create(guild_id, &self.config).await;
        let count = tracks.len();
        for t in tracks {
            s.enqueue(t).map_err(|e| e.to_string())?;
        }
        let started = s.player.current.is_none();
        if started {
            let _ = s.player.start_next();
        }
        self.sessions.replace(s).await;
